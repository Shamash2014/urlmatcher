...
(defn match-query
   "Match on query using pattern and query url
   Fallback to just plain equals if any of the binds was specified"
  [pattern url]
...
(defn unite
  "Unite patterns and url
  Will run appropriate checks, depending on patterns and url supplied"
  [patterns url]
  (let [
         host-match (= (:host patterns) (:host url))]
   (when (true? host-match)

     (let [path-match (match-query (:path patterns) (:path url))
...
(defrecord Pattern [matching]
  IRecogniseable (recognize [_pattern url]
                            (let [pattern-map (build-patterns matching)
                              ...
                              (unite
                                pattern-map
                                (zipmap [:host :path :query] [host path query])))))
